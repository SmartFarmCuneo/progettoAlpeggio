...
def calcola_perimetro_da_coordinate(host='localhost', user='your_username', password='your_password', database='alpeggio', area_id=1):
    try:
        ...
        if result is not None:
            ...
            perimetro_area = [
                [int((lat - lat0) * 11132000), int((lon - lon0) * 11132000)] for lat, lon in coordinates]
...
class DroneController:
    ...
    def vola_all_interno_area(self, perimetro, spacing=50):
        ...
        for i, riga in enumerate(righe):
            ...
            riga_ordinata = sorted(
                riga, key=lambda p: p[0], reverse=(i % 2 == 1))
        ...
        try:
            for punto in percorso:
                ...
                # avvia la lettura del video se non già attivo
                frame_read = self.drone.get_frame_read()
                image = frame_read.frame
                self.captured_images.append(image)
                # Salva la foto nella cartella immagini/drone
                filename = os.path.join(
                    self.image_dir, f"immagine_{punto[0]}_{punto[1]}.jpg")
        ...
        if len(self.captured_images) > 1:
            ...
            stitcher = cv2.Stitcher_create() if hasattr(
                cv2, 'Stitcher_create') else cv2.createStitcher()
            status, stitched_image = stitcher.stitch(self.captured_images)
            if status == cv2.Stitcher_OK:
                final_filename = os.path.join(
                    self.image_dir, "immagine_finale.jpg")
                cv2.imwrite(final_filename, stitched_image)
                print(
                    f"Immagine finale creata e salvata come '{final_filename}'")
            else:
                print(
                    "Errore durante la creazione dell'immagine finale. Codice di status:", status)
        else:
